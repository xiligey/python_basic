...
t1 = threading.Thread(target=run_semaphore, args=('t1',))
t2 = threading.Thread(target=run_semaphore, args=('t2',))
t1.start()
t2.start()
...
# %% ---------------5.守护线程---------------
# 线程守护：主线程不管该子线程的执行情况，只要其他子线程结束且主线程执行完毕，主线程都会关闭。也就是说:主线程不等待该守护线程的执行完再去关闭
# 下面这个例子，这里使用setDaemon(True)把所有的子线程都变成了主线程的守护线程
t5 = threading.Thread(target=run_semaphore, args=('t5',))
t5.setDaemon(True)
t5.start()
print('end')
...
t6 = threading.Thread(target=run_semaphore, args=('t6',))
...
# 所以出现了线程锁，即同一时刻只允许一个线程执行操作。线程锁用于锁定资源，可以定义多个锁，像下面的代码，当需要独占
...
# %% ---------------9.递归锁---------------
# RLock类的用法和Lock类一模一样，但它支持嵌套，在多个锁没有释放的时候一般会实用RLock类
